...
본 시스템은 WebSocket 기반 실시간 상태 동기화 구조와 Git 저장소 기반 버전 관리 구조를 함께 활용하여 협업 기능을 제공한다.

기존 협업 시스템은 대부분 중앙 서버 기반의 실시간 동기화 구조를 사용한다. 이러한 방식은 높은 실시간성을 제공하지만, 복잡한 충돌 해결 알고리즘과 중앙 집중형 상태 관리 구조가 필요하다.

반면 본 시스템은 다음 두 가지 구조를 결합하여 사용한다.

- WebSocket 서버 기반 실시간 상태 공유
- Git 저장소 기반 문서 관리 및 데이터 저장

이를 통해 실시간 협업 기능과 Git 기반 데이터 관리 기능을 동시에 제공하는 것을 목표로 한다.
...
본 시스템은 클라이언트, WebSocket 서버, Git 저장소 간 상호작용을 통해 동작한다.

사용자는 클라이언트 애플리케이션에서 문서를 수정하며, 실시간 상태 정보는 WebSocket 서버를 통해 즉시 공유된다. 문서의 실시간 동기화와 상태 정보 공유는 Yorkie를 사용하여 동기화된다.

반면 문서 데이터는 Git 저장소를 기준으로 commit, push, pull 과정을 통해 저장 및 최종 동기화된다.

시스템은 다음 외부 환경과 상호작용한다.

| 외부 시스템 | 상호작용 내용 |
| --- | --- |
| Git 저장소 | 문서 변경 이력 저장 및 동기화, 첨부 파일 저장 |
| LAN 네트워크 | 사용자 간 데이터 동기화 |
| WebSocket 서버 | 실시간 상태 공유 |
| Yorkie | 실시간 데이터 동기화 서버 및 SDK |
| 파일 시스템 | WebSocket 서버의 데이터 저장 |
...
graph LR
    ...
    %% 데이터 흐름 및 연결 정의
    Client1 <-->|WebSocket| WebSocketServer
    Client2 <-->|WebSocket| WebSocketServer
    WebSocketServer <-->|I/O 연산| FS
    FS <-->|버전 관리| Repo
...
본 시스템은 실시간 동기화 구조와 버전 관리를 위해 다음 두 가지 구조를 결합하여 사용한다.

- WebSocket 서버 기반 실시간 상태 공유
- Git 로컬 저장소 기반 문서 관리 및 데이터 저장
...
본 개발대상은 다음 주요 기능 범위를 포함한다.

- 워크스페이스 기반 협업
- WebSocket 기반 실시간 상태 공유
- Git 저장소 기반 문서 저장 및 동기화
- 포커스 팔로잉
- 플로팅 뷰
...
본 시스템은 클라이언트, Yorkie, WebSocket 서버, Git 저장소 간 상호작용을 통해 동작한다.
...
graph LR
    ...
    %% 애플리케이션 서버 정의
    AppServer[Application / WS Server]
    
    %% Git 저장소 정의
    Repo[(Git 저장소)]

    %% 데이터 흐름 및 연결 정의
    %% 1. 실시간 동기화 흐름 (Yorkie 담당)
    Client1 <-->|gRPC / 실시간 동기화| Y_Server
    Client2 <-->|gRPC / 실시간 동기화| Y_Server

    %% 2. 비즈니스 로직 및 Git 관리 흐름
    Client1 <-->|API / 웹소켓 요청| AppServer
    Client2 <-->|API / 웹소켓 요청| AppServer
    
    AppServer -->|데이터 관리| Repo
    
    %% Yorkie와 웹서버 간의 동기화 연동
    Y_Server -.->|문서 스냅샷 공유| AppServer
...
사용자는 클라이언트 애플리케이션에서 문서를 수정하며, 실시간 상태 정보는 WebSocket 서버를 통해 즉시 공유된다.

반면 문서 데이터는 Git 로컬 저장소를 기준으로 commit 과정을 통해 저장 및 동기화된다.
...
| 기타 고려 사항 |   • 동시 편집 시 충돌 방지 기능이 필요하다.
...
| FR-022-06 | 시스템은 블록의 점유 상태와 점유한 사용자를 다른 사용자 화면에 실시간으로 표시해야 한다. | UC-022 |
| FR-022-09 | 시스템은 생성, 수정, 삭제, 이동된 블록 정보를 다른 사용자 화면에 실시간으로 반영해야 한다. | UC-022 |
| FR-022-12 | 사용자는 네트워크 끊김 동안 점유 중인 블록을 계속 편집할 수 있어야 하며 시스템은 재연결 시 해당 편집을 재동기화해야 한다. | UC-022 |
