...
본 시스템은 CRDT 라이브러리 Yorkie의 실시간 문서 동기화 기능과 Yorkie 리비전 기반 변경 이력 관리 구조를 사용하여 협업 기능을 제공한다.

기존 협업 시스템은 대부분 클라우드에 데이터를 저장하고 WAN기반 실시간 동기화 구조를 사용한다.

하지만 본 시스템은 클라우드가 아닌 호스트 유저의 로컬에 데이터를 저장하고 동일 LAN 안에서 호스트가 Yorkie 서버와 Application / Websocket Server를 직접 구동하는 구조를 전제로 한다.

본 시스템은 다음 두 가지 구조를 결합하여 사용한다.

- Yorkie 기반 실시간 문서 동기화 및 Presence 상태 공유
- Yorkie 서버(MongoDB 백엔드) 기반 문서 영속 및 리비전 기반 변경 이력 기록

이를 통해 실시간 협업 기능과 Yorkie 리비전 기반 변경 로그 관리 기능을 동시에 제공하는 것을 목표로 한다.
...
- Yorkie 서버(MongoDB 백엔드) 기반 문서 영속 및 리비전 기반 변경 이력 기록
- 호스트 로컬 JSON 파일(`.data/`) 기반 애플리케이션 상태 저장(채팅, 워크스페이스 메타데이터, 인증 레코드)
- 블록 단위 점유 상태 표시
- 사용자 추적 및 발표 모드
- 워크스페이스 내 파일 공유
- 실시간 채팅
...
본 시스템은 클라이언트 Yorkie-js-SDK, Yorkie server, Yorkie의 MongoDB 저장소, Application / WS Server 간 상호작용을 통해 동작한다.

사용자는 클라이언트 애플리케이션에서 문서를 수정하며, 문서 내용의 실시간 동기화와 충돌 해소는 Yorkie를 통해 수행된다. Application / WS Server는 워크스페이스 관리, 파일 관리, 권한 판단, 애플리케이션 상태 관리 등 협업에 필요한 비즈니스 로직을 처리한다.

문서 데이터의 영속과 변경 이력은 Yorkie 서버가 담당한다. Yorkie는 MongoDB를 백엔드 저장소로 구동되어 문서 상태를 영속하며, 시스템은 Yorkie의 리비전 API를 통해 변경 이력을 기록하고 복원한다. 따라서 문서 본문은 별도의 파일로 기록되지 않는다.

MongoDB는 Yorkie 서버의 내부 저장소이며 Application / WS Server는 여기에 직접 접근하지 않는다. Application / WS Server가 소유하는 상태(채팅 기록, 워크스페이스 메타데이터, 인증 레코드)는 호스트 로컬의 JSON 파일(`.data/`)에 저장된다.

시스템은 다음 외부 환경과 상호작용한다.

| 외부 시스템 | 상호작용 내용 |
| --- | --- |
| MongoDB (Yorkie 내부 저장소) | 문서 상태 및 리비전 영속. Yorkie 서버만 접근한다 |
| LAN 네트워크 | 클라이언트와 서버 간 통신 |
| Application / WS Server | 워크스페이스, 파일, 권한, 애플리케이션 상태 등 비즈니스 로직 처리 |
| Yorkie | 실시간 문서 동기화, CRDT 기반 충돌 해소, Presence 상태 공유, 문서 영속 및 리비전 기반 버전 이력 |
| 파일 시스템 | 첨부 파일 및 애플리케이션 상태 JSON(`.data/`)의 로컬 저장 |
...
2. 워크스페이스 관리 컴포넌트
    1. 워크스페이스 생성 및 참가
    2. 게스트 퇴장 및 접근 비밀번호 변경(호스트 권한)
    3. 문서 생성·삭제·이름 변경·이동 및 파일 관리
    4. 파일 업로드 및 다운로드
3. 협업 지원 컴포넌트
    1. 사용자 위치 추적
    2. 발표자 시점 공유
...
graph LR
    ...
    %% 호스트 로컬 애플리케이션 상태 저장소 정의
    AppData[(호스트 로컬 JSON 저장소: .data/)]
    ...
    %% 2. 비즈니스 로직 및 애플리케이션 상태 저장 흐름
    Client1 <-->|API / 웹소켓 요청| W2
    Client2 <-->|API / 웹소켓 요청| W2
    W2 -->|채팅·워크스페이스·인증 상태 저장| AppData

    %% 3. 버전 이력 흐름 — 서버가 Yorkie 리비전 API를 호출한다
    W2 -->|리비전 생성 / 조회 / 복원| Y_Server

    %% (선택적 구조) 서버가 Yorkie 문서 변경을 구독하는 경우 — 유지 여부는 미결
...
본 시스템은 Host가 운영하는 단일 서버를 중심으로 한 서버-클라이언트 구조이므로 서버가 중단되면 실시간 중계, Yorkie의 문서 영속, 애플리케이션 상태 기록이 모두 멈춘다.

### 2.3.2. 의존 관계

**1. Yorkie 시스템**

본 시스템의 실시간 문서 협업 기능과 문서의 영속 저장 및 변경 이력 관리는 모두 오픈소스 실시간 동기화 엔진인 Yorkie에 의존한다.
...
- **문서 영속**: Yorkie 서버는 MongoDB를 백엔드 저장소로 구동된다(`--mongo-connection-uri`). 편집 세션 중 CRDT로 충돌이 해소된 문서 상태는 Yorkie가 MongoDB에 영속하며, 세션 종료 및 서버 재시작 이후에도 이 상태가 유지된다. 별도의 파일 기록이나 지연 쓰기 절차는 존재하지 않는다. Yorkie를 인메모리 저장소(MemDB)로 구동하면 재시작 시 문서가 소실되므로 MongoDB 구동은 선택이 아니라 전제이다.
- **버전 이력**: 문서의 버전 이력은 Yorkie의 리비전 API(`createRevision` / `listRevisions` / `getRevision` / `restoreRevision`)에 의존한다. 리비전 스냅샷은 YSON 형식으로 보존되며, 리비전 역시 MongoDB에 영속된다.
- MongoDB는 Yorkie 서버의 내부 저장소이다. Application / WS Server는 MongoDB에 직접 접근하지 않으며, 문서 상태와 리비전에는 오직 Yorkie를 통해서만 접근한다.

**2. 호스트 로컬 파일 시스템**

본 시스템은 Application / WS Server가 소유하는 상태의 영속 저장을 위해 호스트의 로컬 파일 시스템에 의존한다.

- 채팅 기록, 워크스페이스 메타데이터, 인증 레코드는 호스트 로컬의 JSON 파일(`.data/`)에 저장된다.
- 이 상태는 Yorkie의 CRDT 문서가 아니므로 MongoDB에 저장되지 않으며, 문서 영속 경로와 완전히 분리되어 있다.
- 서버 재시작 이후 워크스페이스를 복원하려면 문서 측(Yorkie/MongoDB)과 애플리케이션 상태 측(`.data/`)이 모두 보존되어 있어야 한다.
...
| 액터 | 구분 | 정의 | 관련 유즈케이스 |
| --- | --- | --- | --- |
| 사용자(User) | 상위 주체 | 워크스페이스에서 협업하는 모든 주체 | UC-021, 022, 023, 040, 050, 060, 061, 070, 080 |
| 호스트(Host) | 사용자의 역할 | 서버를 개설·운영하는 사용자 | UC-010, 011 |
...
| 항목 | 내용 |
| --- | --- |
| 유즈케이스명 | 워크스페이스 관리 (게스트 퇴장·비밀번호 변경) |
| ID | UC-011 |
| 작성일 | 2026-08-01 |
| 우선순위 | 높음 |
...
| --- | --- |
| 액터 | 호스트 |
| 개요 | 호스트가 게스트를 워크스페이스에서 퇴장시키거나 워크스페이스 접근 비밀번호를 변경한다. |
| 사전 조건 |   • 호스트가 워크스페이스를 개설해 운영 중이다(UC-010). |
| 기본 흐름 |   1. 호스트가 접속자 목록에서 게스트를 선택하고 "퇴장" 버튼을 클릭한다.
  2. 시스템은 퇴장 여부를 재확인하는 팝업을 표시한다.
  3. 호스트가 퇴장을 확정한다.
  4. 시스템은 해당 게스트의 연결을 종료하고 워크스페이스에서 제외한다.
  5. 시스템은 퇴장된 게스트 화면에 "워크스페이스에서 퇴장되었습니다" 메시지를 표시하고 참가 화면으로 이동시킨다.
  6. 시스템은 다른 사용자의 접속자 목록에서 퇴장된 게스트를 제거한다. |
| 대안 흐름 | **E1. 비밀번호 변경**
  1. 호스트가 워크스페이스 설정에서 "비밀번호 변경"을 선택하고 새 비밀번호를 입력한다.
  2. 시스템은 워크스페이스 접근 비밀번호를 변경한다.
  3. 시스템은 현재 접속 중인 사용자의 세션은 유지하며 재인증을 요구하지 않는다.
  4. 퇴장되었거나 연결이 끊어져 새로 참가를 시도하는 사용자는 변경된 비밀번호를 입력해야 워크스페이스에 입장할 수 있다(UC-020). |
| 결과 |   • 퇴장된 게스트는 워크스페이스에서 제외되고, 재입장하려면 새로 참가 절차를 거쳐야 한다.
  • 비밀번호 변경 후에도 기존 접속자는 영향을 받지 않으며, 새로 참가하는 사용자만 변경된 비밀번호가 필요하다. |
| 비고 |   • 게스트 퇴장과 비밀번호 변경 모두 호스트만 수행할 수 있다. |
...
**E4. 연결 끊김 (유예 시간 이내)
  ...
  3. 사용자 1의 연결이 복구된다.
  4. 사용자 1의 변경 내용이 시스템에 전달된다.
  5. 시스템은 수정된 블록의 텍스트 내용, 블록 위치 정보, 수정 시간 정보를 다른 사용자에게 실시간으로 전달한다.
  6. 시스템은 다른 사용자 화면에 수정된 블록 정보를 실시간으로 표시한다.

**E5. 파일 블록 업로드**
  1. 사용자 1이 '/'로 파일 블록을 선택하거나 파일을 문서에 드래그 앤 드롭한다.
  2. 시스템은 업로드할 파일 선택 창을 표시한다(드래그 앤 드롭 시 생략).
  3. 사용자 1이 업로드할 파일을 선택한다.
  4. 시스템은 파일을 서버 파일 시스템에 저장하고 파일 종류(이미지/PDF/Word/PPT/Excel)에 맞는 블록을 생성한다.
  5. 시스템은 생성된 파일 블록 정보를 다른 사용자에게 실시간으로 전달한다.
  6. 시스템은 다른 사용자 화면에 생성된 파일 블록을 표시한다. |
| 결과 |  • 수정된 블록의 정보(블록 위치, 블록의 텍스트 내용, 수정 시간 정보)를 모든 사용자의 화면에 실시간으로 표시한다. |
| 비고 |  • 유예 시간은 30000ms(30초)로 설정한다. |

## UC-023

| 항목 | 내용 |
| --- | --- |
| 유즈케이스명 | 문서 삭제·이름 변경·이동 |
| ID | UC-023 |
| 작성일 | 2026-08-01 |
| 우선순위 | 높음 |
...
| --- | --- |
| 액터 | 사용자 |
| 개요 | 사용자가 문서 트리에 있는 문서의 이름을 변경하거나 다른 위치로 이동하거나 삭제한다. |
| 사전 조건 |   • 사용자가 워크스페이스에 접속 중이다.
  • 대상 문서가 워크스페이스 문서 트리에 존재한다. |
| 기본 흐름 |   1. 사용자가 문서 트리에서 대상 문서의 메뉴를 연다.
  2. 시스템은 이름 변경, 이동, 삭제 메뉴를 표시한다.
  3. 사용자가 "이름 변경"을 선택하고 새 이름을 입력한다.
  4. 시스템은 문서 이름을 변경하고 문서 트리에 반영한다.
  5. 시스템은 변경된 문서 이름을 다른 접속 사용자의 문서 트리에 실시간으로 전달한다. |
| 대안 흐름 | **E3-1. 문서 이동**
  1. 사용자가 "이동"을 선택하고 대상 폴더를 지정한다.
  2. 시스템은 문서를 지정된 폴더 하위로 이동하고 문서 트리에 반영한다.
  3. 시스템은 변경된 문서 위치를 다른 접속 사용자의 문서 트리에 실시간으로 전달한다.

**E3-2. 문서 삭제**
  1. 사용자가 "삭제"를 선택한다.
  2. 시스템은 삭제 여부를 재확인하는 팝업을 표시한다.
  3. 사용자가 삭제를 확정한다.
  4. 시스템은 문서를 문서 트리에서 제거하며, 하위 문서가 있으면 함께 제거한다.
  5. 시스템은 삭제된 문서 정보를 다른 접속 사용자의 문서 트리에 실시간으로 반영한다.

**E4a. 이름 중복**
  1. 시스템은 변경하려는 이름이 동일 위치 내에서 중복되면 오류 메시지를 표시하고 재입력을 요청한다. |
| 결과 |   • 문서의 이름, 위치, 존재 여부가 변경되어 모든 접속 사용자의 문서 트리에 실시간으로 반영된다. |
| 비고 |   • 하위 문서가 있는 문서를 삭제하면 하위 문서도 함께 삭제된다.
  • 문서가 삭제되어도 삭제 전에 생성된 Yorkie revision은 ID로 조회 가능하다. 단, 삭제된 문서의 revision 목록은 다시 조회할 수 없다 — revision ID를 별도로 보관하지 않으면 접근할 수 없다. |
...
| 기본 흐름 |   1. 발표자가 "공유하기" 버튼을 클릭한다.
  ...
  3. 시스템은 워크스페이스 팔로워 화면에 "발표자가 화면 공유를 시작했습니다" 알림과 "참여하기" 버튼을 표시한다.
  4. 팔로워가 "참여하기" 버튼을 클릭한다.
  5. 시스템은 팔로워의 화면을 발표자의 현재 문서 및 화면 위치로 이동시키고 발표 모드 상태로 전환한다.
  6. 시스템은 발표 모드에 참여한 사용자의 문서 수정 기능과 개별 화면 이동 기능을 제한한다.
  7. 발표자가 문서를 이동하거나 화면 위치 및 스크롤 위치를 변경한다.
  8. 시스템은 변경된 문서 ID, 화면 위치 정보, 스크롤 위치 정보를 팔로워에게 실시간으로 전달한다.
  9. 시스템은 팔로워 화면을 발표자의 화면 위치와 동일하게 표시한다. |
| 대안 흐름 | **E3-1. 팔로워의 일시 정지**
  1. 팔로워가 "일시 정지" 버튼을 클릭한다.
  2. 시스템은 팔로워의 발표 모드 상태를 해제한다.
  3. 시스템은 팔로워의 화면 시점 고정을 해제하고 문서 수정 및 화면 이동 기능을 허용한다.
  4. 팔로워가 "재개" 버튼을 클릭한다.
  5. 시스템은 팔로워의 발표 모드 상태를 다시 활성화한다.
  6. 시스템은 팔로워의 화면을 발표자의 현재 문서 및 화면 위치 정보와 다시 동기화한다.

**E3-2. 팔로워의 포커스 팔로잉 종료**
  1. 팔로워가 "종료" 버튼을 클릭한다.
  2. 시스템은 팔로워의 발표 모드 상태를 종료한다.
  3. 시스템은 팔로워의 시점 고정 및 문서 수정 제한을 해제한다.

**E3-3. 발표자의 접속 종료**
  1. 발표자의 연결이 종료되거나 발표자가 발표를 종료한다.
  2. 시스템은 발표 세션을 종료한다.
  3. 시스템은 팔로워 화면에 "공유가 종료되었습니다." 메시지를 표시한다.
  4. 시스템은 팔로워의 화면 시점 고정 및 문서 수정 제한 상태를 해제한다.

**E3-4. 발표자 도구**
  1. 발표자가 발표 도구를 사용하여 특정 문서 영역 또는 블록을 강조한다.
  2. 시스템은 강조 대상의 문서 ID, 블록 위치 정보, 강조 상태 정보를 팔로워에게 실시간으로 전달한다.
  3. 시스템은 팔로워 화면에 강조 표시를 적용한다.
  4. 발표가 종료되면 시스템은 강조 관련 메타데이터를 제거하고 팔로워 화면의 강조 표시를 해제한다. |
| 결과 |   • 팔로워는 발표자의 발표 위치를 실시간으로 확인할 수 있다.
...
| 세부 내용 |   • 시스템은 워크스페이스 이름 입력란과 접근 비밀번호 입력란을 포함한 초기 설정 화면을 제공해야 한다.
  • 사용자는 워크스페이스 개설 설정을 완료하고 생성 요청을 수행할 수 있어야 한다.
...
| 요구사항 분류 | 인터페이스 |
| 관련 유즈케이스 | 문서 생성(UC-021), 문서 작업(UC-022), 문서 삭제·이름 변경·이동(UC-023) |
| 중요도 | 높음 |
...
| 세부 내용 |   • 사용자는 블록 클릭 또는 '/' 입력으로 블록을 생성할 수 있어야 한다.
  ...
  • 사용자는 문서 트리에서 문서의 이름을 변경, 이동, 삭제할 수 있어야 한다. |
...
| --- | --- |
| 요구사항 고유번호 | SIR009 |
| 요구사항 명칭 | 사용자 추적 인터페이스 |
| 요구사항 분류 | 인터페이스 |
| 관련 유즈케이스 | 사용자 추적(UC-040) |
| 중요도 | 보통 |
| 요구사항 상세설명 | 사용자가 워크스페이스 내 다른 사용자의 현재 작업 위치를 확인하고 해당 위치로 이동할 수 있는 인터페이스를 제공한다. |
| 세부 내용 |   • 시스템은 현재 접속 중인 사용자 목록과 각 사용자의 작업 문서 정보를 표시해야 한다.
  • 사용자는 목록에서 다른 사용자를 선택해 해당 사용자의 작업 위치로 이동할 수 있어야 한다.
  • 사용자는 이동 전 작업 위치로 복귀할 수 있어야 한다. |
| 기타 고려 사항 |   • 시스템은 미접속 사용자를 비활성(흐림) 상태로 표시하고 선택을 차단해야 한다. |
| 산출 정보 |   • 사용자 목록
  • 사용자별 작업 문서 정보
  • 화면 위치 정보 |
...
| --- | --- |
| 요구사항 고유번호 | SIR010 |
| 요구사항 명칭 | 채팅 내 파일 관리 인터페이스 |
| 요구사항 분류 | 인터페이스 |
| 관련 유즈케이스 | 채팅 내 파일 관리(UC-061) |
| 중요도 | 보통 |
| 요구사항 상세설명 | 사용자가 채팅으로 주고받은 파일과 링크를 종류별로 모아 열람·다운로드할 수 있는 인터페이스를 제공한다. |
| 세부 내용 |   • 시스템은 채팅에 공유된 파일과 링크를 종류별 탭으로 분류하여 표시해야 한다.
  • 시스템은 파일 목록에 파일 이름, 종류, 전송 사용자, 전송 시간 정보를 표시해야 한다.
  • 사용자는 파일 미리보기 및 다운로드 기능을 수행할 수 있어야 한다. |
| 기타 고려 사항 |   • 파일 업로드는 채팅 메시지(UC-060)를 통해서만 가능하다. |
| 산출 정보 |   • 파일 정보
  • 전송 사용자 정보
  • 전송 시간 정보 |
...
| --- | --- |
| 요구사항 고유번호 | SIR011 |
| 요구사항 명칭 | 워크스페이스 관리 인터페이스 |
| 요구사항 분류 | 인터페이스 |
| 관련 유즈케이스 | 워크스페이스 관리(UC-011) |
| 중요도 | 높음 |
| 요구사항 상세설명 | 호스트가 접속자 목록에서 게스트를 퇴장시키거나 워크스페이스 접근 비밀번호를 변경할 수 있는 인터페이스를 제공한다. |
| 세부 내용 |   • 시스템은 호스트에게 접속자별 "퇴장" 버튼을 제공해야 한다.
  • 시스템은 게스트 퇴장 전 재확인 팝업을 표시해야 한다.
  • 시스템은 호스트에게 비밀번호 변경 입력란을 제공해야 한다. |
| 기타 고려 사항 |   • 호스트가 아닌 사용자에게는 이 인터페이스가 노출되지 않아야 한다. |
| 산출 정보 |   • 접속자 목록
  • 워크스페이스 비밀번호 |
...
| 세부 내용 |   • 시스템은 웹 브라우저로부터 사용자 접속 요청 및 워크스페이스 참가 요청을 수신해야 한다.
  ...
  • 시스템은 실시간 통신을 통해 문서 변경 사항 및 사용자 상태 정보를 브라우저에 실시간 동기화해야 한다. 문서 변경과 사용자 상태는 Yorkie의 gRPC-Web 서버 스트리밍(WatchDocument)으로 전달되며, 그 외 워크스페이스 수준 알림은 App/WS 서버의 WebSocket을 사용한다.
...
| 요구사항 분류 | 소프트웨어 인터페이스 |
| 중요도 | 높음 |
| 요구사항 상세설명 | 시스템은 Yorkie 서버의 리비전 API와 연동하여 문서 상태를 영속하고 문서 버전 이력을 관리할 수 있어야 한다. |
| 세부 내용 |   • 시스템은 Yorkie 서버(MongoDB 백엔드)를 통해 문서 상태를 영속할 수 있어야 한다.
  • 시스템은 Yorkie 리비전 API(`createRevision` / `listRevisions` / `getRevision` / `restoreRevision`)를 통해 문서에 대한 버전 이력 관리를 할 수 있어야 한다. |
| 기타 고려 사항 |  |
...
| ID | 기능 요구사항 | 관련 UC |
| --- | --- | --- |
| FR-011-01 | 호스트는 접속자 목록에서 게스트를 선택하여 워크스페이스에서 퇴장시킬 수 있어야 한다. | UC-011 |
| FR-011-02 | 시스템은 게스트 퇴장 전 퇴장 여부를 재확인해야 한다. | UC-011 |
| FR-011-03 | 시스템은 퇴장된 게스트의 연결을 종료하고 다른 사용자의 접속자 목록에서 제거해야 한다. | UC-011 |
| FR-011-04 | 호스트는 워크스페이스 접근 비밀번호를 변경할 수 있어야 한다. | UC-011 |
| FR-011-05 | 시스템은 비밀번호 변경 시 현재 접속 중인 사용자의 세션을 유지해야 한다. | UC-011 |
| FR-011-06 | 시스템은 비밀번호 변경 이후 새로 참가를 시도하는 사용자에게 변경된 비밀번호를 요구해야 한다. | UC-011 |
| FR-011-07 | 시스템은 호스트가 아닌 사용자가 게스트 퇴장 또는 비밀번호 변경을 요청하면 이를 거부해야 한다. | UC-011 |
...
| FR-022-13 | 사용자는 파일을 업로드하여 문서에 파일 블록으로 추가할 수 있어야 한다. | UC-022 |
| FR-022-14 | 시스템은 업로드된 파일의 종류(이미지/PDF/Word/PPT/Excel)에 맞는 블록을 생성해야 한다. | UC-022 |

번호 결치(05, 07, 08, 10, 11)는 의도된 것으로 확인됨 — 누락된 요구사항 없음.

---

### 3.3.6. 문서 삭제·이름 변경·이동 기능

| ID | 기능 요구사항 | 관련 UC |
| --- | --- | --- |
| FR-023-01 | 사용자는 문서의 이름을 변경할 수 있어야 한다. | UC-023 |
| FR-023-02 | 시스템은 변경하려는 문서 이름이 동일 위치 내에서 중복되면 오류를 표시하고 재입력을 요청해야 한다. | UC-023 |
| FR-023-03 | 사용자는 문서를 다른 폴더 위치로 이동할 수 있어야 한다. | UC-023 |
| FR-023-04 | 사용자는 문서를 삭제할 수 있어야 한다. | UC-023 |
| FR-023-05 | 시스템은 문서 삭제 전 삭제 여부를 재확인해야 한다. | UC-023 |
| FR-023-06 | 시스템은 하위 문서가 있는 문서를 삭제할 때 하위 문서도 함께 삭제해야 한다. | UC-023 |
| FR-023-07 | 시스템은 문서의 이름 변경, 이동, 삭제 결과를 다른 접속 사용자의 문서 트리에 실시간으로 반영해야 한다. | UC-023 |
...
| FR-060-07 | 시스템은 채팅 전송 실패 시 사용자에게 실패 메시지를 표시해야 한다. | UC-060 |

---

### 3.3.11. 채팅 내 파일 관리 기능
...
| NFR-SAF-002 | 시스템은 문서 또는 파일 삭제 시 삭제 여부를 재확인해야 한다. | UC-022, UC-023 |
| NFR-SAF-003 | 시스템은 비정상 종료 발생 시 Yorkie가 MongoDB에 마지막으로 영속한 문서 상태를 기준으로 문서를 복구할 수 있어야 한다. | UC-022 |
...
| 목록 블록 | 순서 목록 및 비순서 목록을 표현하기 위한 블록. 항목을 들여쓰기하여 중첩(하위 목록)할 수 있다 |
