...
본 시스템은 실시간 문서 협업과 변경 이력 관리를 위해 다음 두 가지 구조를 결합하여 사용한다.
...
graph LR
    ...
    AppServer -->|지연 쓰기 후 commit| Repo
    
    %% Yorkie와 웹서버 간의 동기화 연동
    Y_Server -.->|문서 상태 제공| AppServer
...
사용자는 클라이언트 애플리케이션에서 문서를 수정하며, 문서 내용과 사용자 상태는 Yorkie를 통해 실시간으로 동기화된다.

문서 데이터는 실시간 편집 중 Git으로 직접 동기화되지 않는다. 시스템은 Yorkie의 문서 상태를 일정 시점에 서버 파일 시스템에 기록하고, 해당 변경분을 서버 로컬 Git 저장소에 commit하여 변경 이력 로그로 보존한다.
