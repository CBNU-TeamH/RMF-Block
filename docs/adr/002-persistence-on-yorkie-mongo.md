...
- History granularity becomes a deliberate choice — an explicit `createRevision` call — instead of a side effect of a write-debounce window. *Corrected 2026-09-21*: only half of this holds. Yorkie's own auto-revision records a revision on every snapshot, so the baseline granularity is a side effect of `snapshotInterval` after all — much the shape this bullet claimed to escape. What the explicit call buys is the ability to add *named* revisions on top of that baseline, not to be the only thing producing one.
...
- **Open: what triggers a revision.** *Corrected 2026-09-21* — this item was written on the premise that "Yorkie never snapshots on its own," which is false. Measured 2026-09-21 against this project's own stack: the default Yorkie project runs with `autoRevisionEnabled` on and `snapshotInterval`/`snapshotThreshold` at 500, so Yorkie records a revision every time it snapshots a document, with no app code involved. `createRevision` is still an explicit call, but it is no longer the only thing that produces a revision. What is actually open is narrower again: whether the app should add *named* revisions on a user action, and the fact that a "before restore" safety revision must be app-created, since `restoreRevision` does not make one. No FR or UC currently covers version history at all, so nothing forces the answer yet (issue #23).
