...
import { inlineHeaders, isInlineType } from "@/lib/files/serving";
import { InvalidFileIdError } from "@/lib/files/types";

/**
 * A file the browser may render in place: an image, so a chat bubble can hold
 * `<img src=".../preview">` (FR-050-03, and the inline half of FR-060-02), or
 * a PDF, so a PDF block can hold an `<iframe>` of it (FR-080-01~03).
 *
 * This is the endpoint that has to name a real type, so it is the one that can
 * be wrong. It refuses everything it cannot prove is safe: not on the list, no
 * preview. The rest of the reasoning is in `lib/files/serving.ts`.
 *
...
export async function GET(
  ...
  if (!file || !isInlineType(file.type)) {
  ...
  try {
    ...
    return new Response(new Uint8Array(bytes), {
      headers: inlineHeaders(file.type, file.name),
    });
  } catch (error) {
