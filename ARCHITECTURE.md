...
graph TB
  ...
  BizLogic --> DataFiles
  BizLogic --> SessionReg
  BizLogic -->|"createRevision / listRevisions<br/>(Yorkie client SDK, version history only)"| YorkieCore
...
- WebSocket 업그레이드(`/api/chat/ws`, `/api/workspace/ws` 둘 다)는 세션 또는 호스트 시크릿을 통과해야만 완료되고, 실패하면 핸드셰이크 전에 401로 거부됨 — 인증되지 않은 연결은 `WSHub`에 등록조차 되지 않음
- App/WS 서버는 Yorkie 클라이언트 SDK의 리비전 메서드(`createRevision`/`listRevisions`) — 별도 admin API가 아니라 attach된 `Document`를 다루는 client SDK 메서드 — 로 버전 이력만 가져오고, 문서 본문 저장은 전적으로 Yorkie(MongoDB 기반)에 위임 — 앱은 Mongo에 직접 연결하지 않음
