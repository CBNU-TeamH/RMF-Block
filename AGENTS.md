...
- **Before merging**: `pnpm lint`, `pnpm test` and `pnpm build` pass. CI enforces this — `lint · test · build`, `container smoke test` and `comment budget` are required checks on `main`, and all run on every PR.
