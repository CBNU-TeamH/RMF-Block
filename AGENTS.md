...
- **Before merging**: `pnpm lint`, `pnpm test` and `pnpm build` pass. CI enforces this — `lint · test · build`, `container smoke test` and `yorkie invariants` are required checks on `main`. `comment budget` runs on every PR too but is deliberately not required: a red run is a signal to act on, not a merge block.
...
- [ ] Measure the review-quality criteria `comment-budget`'s promotion left behind — whether `/code-review` ever cites `docs/conventions.md`, whether S-1..S-5 catch anything in a real review, and what a review costs. The CI job itself shipped on 2026-09-23 (`comment budget` runs on every PR, kept out of the required checks); these three never had data and are tracked in issue #92.
