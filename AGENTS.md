...
- **Before merging**: `pnpm lint`, `pnpm test` and `pnpm build` pass. CI enforces this — `lint · test · build`, `container smoke test`, `comment budget` and `yorkie invariants` are required checks on `main`, and all run on every PR.
...
- [ ] Measure the review-quality criteria `comment-budget`'s promotion left behind — whether `/code-review` ever cites `docs/conventions.md`, whether S-1..S-5 catch anything in a real review, and what a review costs. The promotion itself shipped on 2026-09-23 (`comment budget` is a required CI check); these three never had data and are tracked in issue #92.
