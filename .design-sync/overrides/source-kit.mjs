// forked from design-sync lib/source-kit.mjs - skip Next.js dynamic-route segments ([id]) when deriving a component's group
    ...
    for (const c of components) {
      ...
      c.group = slug(
        slash(relative(srcRoot, dirname(hit)))
          .split('/')
          .filter((s) => s && !/^\[.*\]$/.test(s) && s.toLowerCase() !== c.name.toLowerCase() && !GENERIC_DIR.has(s.toLowerCase()))
          .at(-1)
